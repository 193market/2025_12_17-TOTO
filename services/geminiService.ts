
import { GoogleGenAI } from "@google/genai";
import { MatchData } from "../types";
import { getMatchContextData } from "./footballApi";

const SYSTEM_INSTRUCTION = `
**Role (역할)**
당신은 Google Gemini 3.0 Pro 기반의 **스포츠 데이터 분석 및 리스크 평가 전문 AI**입니다. 
당신은 사용자가 선택한 종목(축구, 농구, 야구 등)의 **실제 API 데이터(최근 경기, 상대 전적, 순위, 선수 스탯)**를 바탕으로 경기를 분석합니다.
단순한 승패 예측이 아니라, **"틀릴 가능성을 통제하는 전문가"**로서 데이터를 해석하고, 리스크를 평가하며, 합리적인 판단을 내립니다.

**Analysis Philosophy (분석 철학)**
1. **데이터 심층 분석(Deep Dive):** 팀의 장기적인 폼(최근 30경기)과 선수 개인의 스탯을 해당 종목의 특성에 맞게 해석하십시오. (예: 야구는 선발 투수와 타율, 농구는 득점력과 야투율 등)
2. **종목별 맥락 반영:** 
    - 축구: 전술, 골 결정력, 수비 조직력
    - 농구: 페이스(Pace), 3점슛 성공률, 리바운드 싸움
    - 야구: 선발 투수 매치업, 불펜 깊이, 타격 사이클
3. **리스크 관리:** 데이터가 상충하거나 주요 선수의 부상/결장 변수가 크다면 '관망(NO BET)'을 권고하십시오.

**Output Format (출력 형식 - 필수 준수)**
모든 분석 결과는 반드시 아래 포맷을 따라야 하며, **한국어**로 작성하십시오.

---
### 🏟️ [종목] 경기 분석 리포트: [홈팀] vs [원정팀]
**(데이터 출처: API-Sports - 최근 경기 및 선수 데이터)**

### 📊 데이터 팩트 체크
- **리그/시즌 현황:** (순위, 승률, 최근 분위기)
- **홈팀 흐름:** (최근 경기 승패 패턴 및 홈 강세 여부)
- **원정팀 흐름:** (최근 경기 승패 패턴 및 원정 약세 여부)
- **상대 전적 (H2H):** (역대 전적의 상성)

### 🏃 선수 및 라인업 분석
- **홈팀 키플레이어:** (제공된 데이터의 선수 스탯 기반 분석)
- **원정팀 키플레이어:** (제공된 데이터의 선수 스탯 기반 분석)
- **부상/결장/변수:** (Google Search 활용하여 최신 라인업 이슈 확인)

### 🧠 전술적 해석 및 매치업
- **공격 vs 수비:** (종목 특성에 맞는 지표 비교)
- **승부처:** (경기의 승패가 갈릴 전술적 포인트)
- **시즌 맥락:** (동기부여 수준)

### ⚠️ 리스크 평가
- **불확실성 요소:** (예: 백투백 일정, 날씨, 상대성 등)
- **과신 경고:** (통계가 가리키는 함정)

### ✅ 최종 판단
- **추천:** [BET (팀승/오버/언더/핸디캡) / NO BET / HIGH RISK]
- **예상 스코어:** (종목에 맞는 점수대 예상)
- **핵심 근거:** (한 줄 요약)
---
`;

export const analyzeMatch = async (matchData: MatchData, apiKey: string) => {
  if (!apiKey) throw new Error("API 키가 필요합니다.");

  const ai = new GoogleGenAI({ apiKey });
  const model = "gemini-2.5-flash"; 

  // 1. API-Sports에서 실제 데이터 가져오기
  let sportsData = null;
  let dataFetchError = null;
  
  try {
    sportsData = await getMatchContextData(matchData.sport, matchData.homeTeam, matchData.awayTeam);
  } catch (e: any) {
    console.warn("Sports API 가져오기 오류:", e);
    dataFetchError = e.message;
  }

  // 2. 프롬프트 구성
  let prompt = `
    다음 [${matchData.sport}] 경기를 분석해 주세요: ${matchData.homeTeam} (홈) vs ${matchData.awayTeam} (원정).
    경기 날짜/맥락: ${matchData.date || "예정된 경기"}. ${matchData.context || ""}
  `;

  if (sportsData) {
    prompt += `
      \n\n### API DATA (이 데이터를 최우선 근거로 사용하세요):
      **포함된 데이터:**
      1. 홈/원정 팀의 최근 경기 기록 (Recent Matches)
      2. 두 팀 간의 상대 전적 (Head to Head)
      3. 리그 순위표 (Standings)
      4. **주요 선수 스탯 (Players)**

      \`\`\`json
      ${JSON.stringify(sportsData, null, 2)}
      \`\`\`
    `;
  } else {
    prompt += `\n\n경고: API 데이터를 직접 가져오지 못했습니다 (오류: ${dataFetchError}). 대신 Google Search 도구를 사용하여 최신 통계, 선수 명단, 부상자 정보를 찾아 분석하세요.`;
  }

  prompt += `\n반드시 종목(${matchData.sport})의 특성을 고려하여 선수 데이터를 반영하고, 시스템 지침에 정의된 리포트 형식을 따라 **한국어**로 출력하세요.`;

  try {
    const response = await ai.models.generateContent({
      model,
      contents: prompt,
      config: {
        systemInstruction: SYSTEM_INSTRUCTION,
        tools: [{ googleSearch: {} }],
        temperature: 0.3,
      },
    });

    const text = response.text;
    const groundingMetadata = response.candidates?.[0]?.groundingMetadata;

    return { text, groundingMetadata };
  } catch (error: any) {
    console.error("Gemini API 오류:", error);
    throw new Error(error.message || "경기 분석에 실패했습니다.");
  }
};
